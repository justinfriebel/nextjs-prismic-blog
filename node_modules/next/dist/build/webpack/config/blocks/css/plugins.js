...
const pc=plugins;plugins=Object.keys(plugins).reduce((acc,curr)=>{const p=pc[curr];if(typeof p==='undefined'){console.error(getError_NullConfig(curr));throw new Error(genericErrorText);}acc.push([curr,p]);return acc;},[]);}const parsed=[];plugins.forEach(plugin=>{if(plugin==null){console.warn(`${_chalk.default.yellow.bold('Warning')}: A ${_chalk.default.bold('null')} PostCSS plugin was provided. This entry will be ignored.`);}else if(typeof plugin==='string'){parsed.push([plugin,true]);}else if(Array.isArray(plugin)){const pluginName=plugin[0];const pluginConfig=plugin[1];if(typeof pluginName==='string'&&(typeof pluginConfig==='boolean'||typeof pluginConfig==='object')){parsed.push([pluginName,pluginConfig]);}else{if(typeof pluginName!=='string'){console.error(`${_chalk.default.red.bold('Error')}: A PostCSS Plugin must be provided as a ${_chalk.default.bold('string')}. Instead, we got: '${pluginName}'.`);}else{console.error(`${_chalk.default.red.bold('Error')}: A PostCSS Plugin was passed as an array but did not provide its configuration ('${pluginName}').`);}throw new Error(genericErrorText);}}else{console.error(`${_chalk.default.red.bold('Error')}: An unknown PostCSS plugin was provided (${plugin}).`);throw new Error(genericErrorText);}});const resolved=await Promise.all(parsed.map(p=>loadPlugin(dir,p[0],p[1])));const filtered=resolved.filter(Boolean);return filtered;}